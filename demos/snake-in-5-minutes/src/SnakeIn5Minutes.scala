    ...
    case FrameTick =>
      ...
      val nextApple: Point =
        if model.apple == model.head then
          Point(
            context.frame.dice.roll(model.gridSize) - 1,
            context.frame.dice.roll(model.gridSize) - 1
          )
        else model.apple

      val nextTail =
        if (model.trail.contains(model.head)) 5
        else if (model.apple == model.head) model.tail + 1
        else model.tail

      val nextTrail =
        model.head :: model.trail.dropRight(Math.max(0, model.trail.length - model.tail))
